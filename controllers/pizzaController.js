...
class PizzaController {
  async getAllPizzas(req, res) {
    const { category, sorttype, search, pagesize, pagenumber } = req.query;
    let result = pizzas;
    if (category) {
      result = Number(category)
        ? result.filter((item) => Number(item.category) === Number(category))
        : result;
    }
    if (Number(sorttype) === 0) {
      result = result.sort((item1, item2) => (item1.rating < item2.rating ? 1 : -1));
    }
    if (Number(sorttype) === 1) {
      result = result.sort((item1, item2) => (item1.price < item2.price ? 1 : -1));
    }
    if (Number(sorttype) === 2) {
      result = result.sort((item1, item2) => (item1.name > item2.name ? 1 : -1));
    }
    if (search) {
